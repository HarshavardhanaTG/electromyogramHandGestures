...
We also implement supervised minimum distance to mean (MDM) and SVM on the manifold and the 
    ...
    def SVM(self, X, Y):
        ...
        for i in range(numberMatricesX):
            for j in range(numberMatricesY):
                distance = self.matrixDistance.distance(X[i], Y[j])
                pairwiseDistances[i, j] = np.exp(-self.gamma * distance ** 2)
...
class tangentMap:
    ...
    def logMap(self, K, L):
        ...
        cholL = np.linalg.cholesky(L)
        lDiag, lLowerTril = splitMatrix(cholL, self.LOG)
        
        cholK = np.linalg.cholesky(K)
        kDiag, kLowerTril = splitMatrix(cholK, self.LOG)

        lowTril = kLowerTril - lLowerTril
        diag = lDiag @ np.diag(np.log(1/(np.diag(lDiag) * np.diag(kDiag))))
        mapped = lowTril + diag
        return mapped